	...
	public :
		...
		int get_i(uint16_t type_id, uint64_t n_key, uint32_t n_item_key, char* &p_result,
			...
			if(NULL == memc){
				...
				uint16_t total_index = svr_ip_port.ip_index_ + svr_ip_port.port_index_ * ip_num_;
                LOG_ERROR("MC %s:%"PRIu32" GET %d ms", vec_mc_server_[total_index].ip_, 
		...
		int mget(uint16_t type_id, const char* str_keys[], uint32_t keys_num, char* results_array[],
			...
			uint16_t total_index = svr_ip_port.ip_index_ + svr_ip_port.port_index_ * ip_num_;
            LOG_ERROR("MC %s:%"PRIu32" MGET %d ms", vec_mc_server_[total_index].ip_, 
		...
		int set(const string &key, const string &value) {
			...
			get_ip_port(0, svr_ip_port);
			memcached_st* memc = get_mc_raw(svr_ip_port);
			if (NULL == memc) return -1;

			struct timeval tv_begin, tv_end;
			gettimeofday(&tv_begin, NULL);
			memcached_return rc = memcached_set(memc, key.c_str(), key.size(), value.c_str(), value.size(), 0, 0);
			gettimeofday(&tv_end, NULL);
			int time_get =(tv_end.tv_sec - tv_begin.tv_sec) * 1000 + (tv_end.tv_usec - tv_begin.tv_usec) / 1000;

			uint16_t total_index = svr_ip_port.ip_index_ + svr_ip_port.port_index_ * ip_num_;
			LOG_INFO("MC %s:%"PRIu32" SET %s cost %d ms", vec_mc_server_[total_index].ip_, vec_mc_server_[total_index].port_, key.c_str(), time_get);
			if (rc == MEMCACHED_SUCCESS) {
				return 0;
			} else {
				LOG_ERROR("MC %s:%"PRIu32" SET %s fail! %d", vec_mc_server_[total_index].ip_, vec_mc_server_[total_index].port_, key.c_str(), rc);
