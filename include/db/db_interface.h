...
typedef struct _Db_Info_Struct{
	...
	//added by wcp to control mget thread num and type
	uint8_t mget_type_;
	uint8_t mget_thread_num_;

	char redis_password_[PATH_MAX];
	...
	public:
		...
		_Db_Info_Struct(const _Db_Info_Struct& db_info_struct){
			...
			strcpy(redis_password_, db_info_struct.redis_password_);
		}
		...
		int load_config(const char* file_name, const char* db_name){
			...
			read_profile_string(db_name, "REDIS_PASSWORD", redis_password_, PATH_MAX, "", file_name);
			return 1;
...
typedef map<const char*, RedisValue> MapRedisValue;
...
class DbInterface{
	...
	protected:
		...
		int gets_ip_port(const set<string> &keys, MapSplitIpPort& map_ip_port) {
			switch (db_info_struct_.select_type_) {
			case 0:
			{
				// key为数字, 随机取
				...
				for (set<string>::const_iterator it = keys.begin(); it != keys.end(); ++it) {
					uint64_t key = strtoull((*it).c_str(), NULL, 10);
					req_struct.n_requsts_[req_struct.num_] = key;
					req_struct.num_++;
				}
			...
			case 1:
			{
				...
				for (set<string>::const_iterator it = keys.begin(); it != keys.end(); ++it) {
					SvrIpPort svr_ip_port;
					uint64_t key = strtoull((*it).c_str(), NULL, 10);
					int hash_key = (key / 10) % 10;
					svr_ip_port.ip_index_ = hash_key % ip_num_;
					svr_ip_port.port_index_ = port_index;

					MapSplitIpPort::iterator pos = map_ip_port.find(svr_ip_port);
					if (pos != map_ip_port.end()) {
						(*pos).second.n_requsts_[(*pos).second.num_] = key;
						(*pos).second.num_++;
					} else {
						ReqStruct req_struct;
						req_struct.n_requsts_[0] = key;
						req_struct.num_++;
						map_ip_port.insert(make_pair(svr_ip_port, req_struct));
					}
				}
		...
		int get_ip_port(uint64_t n_key,  SvrIpPort& svr_ip_port){
			...
			case 4:
				...
				return 1;
			case 5: // for user offline lushan mc get
				srand(randomNumberSeed());
				n_key_id = n_key;
				//ip_num = (uid/10%24)/4
