    ...
    if( thr != NULL){
		...
		if(p_api_adapter != NULL){
			for(std::vector<uint64_t>::iterator it = thr->vec_keys_.begin();
				...
				sprintf(req_body, "{\"cmd\": \"candidate\", \"body\": {\"obj_id\": \"1145099163\", \"cand_id\": [\"keyword_cand#10\", \"ucf_cand#20\", \"topic_cand#20\"], \"re_query\": 0, \"num\": 50, \"req_id\": \"142051241\", \"tobj_id\": \"4000520874335267\"}, \"api\": \"venus_retrieval\"}");
				int n_out_len = 0;	
...
update_global_handle_data(thread_data_t thread_data_t_array[], int num, char *output,
	...
	p_new_global_db_company->load_config("../conf/global_db_config_main.ini");
...
int load_conf(const char *path) {
	...
	strncpy(g_conf.keys_file_path, woo::conf_get_str(conf, "keys_file_path", "../data/user_list.txt"), 
			sizeof(g_conf.log_path));
